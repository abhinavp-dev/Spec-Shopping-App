...
> Detail Screen (Stage 2) will add `Features/Detail/` following the same pattern.
...
## 6. Component (View) Hierarchy — Stage 1
 ...
 └── RootView                  (observes SessionStore)
      ...
      └── NavigationStack       (if authenticated)
           └── HomeView         (uses HomeViewModel)
                ...
                └── EmptyStateView
                     [tap on ProductRowView → pushes Detail route — stubbed until Stage 2]
```
...
- Pagination handled by incrementing `skip` by `limit` in `HomeViewModel`, appending results, stopping when `skip >= total`.

---

## Changelog
- **Stage 1 (Login + Home):** Initial architecture defined as above.
- **Stage 2 (Detail Screen):** _Pending — will extend Component Hierarchy, Folder Structure (`Features/Detail/`), and API Architecture (product-by-id endpoint) once started._
